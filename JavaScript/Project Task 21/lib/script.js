...
codeIds.forEach(function (codeId) {
  ...
  myCode.appendChild(textnode);
  document.getElementById(codeId).appendChild(myCode); // Remove empty ES5 compatibility

  var list = document.getElementById(percentIds[codeIncrement]);
  list.removeChild(list.childNodes[1]); // Add paragraph with new % compatibility
